...
function preload() {
  ...
  table = loadTable('data/1000_sample_objects.csv', 'csv', 'header');
}
...
function analyzeData(){
  ...
  for(var i=1300; i<2023; i++){
    let yearNow = {};
    yearNow.year = i;
    const re = new RegExp("^\d{4}$");
    let x = re.exec(String(i));
    yearNow.count = table.findRows(String(i),2).length;
...
function drawLabels(){
  ...
  textAlign(LEFT);
  textSize(20);
  text("Number of items in the sample data set", margin,margin-30);
  textSize(10);
  textStyle(NORMAL);
